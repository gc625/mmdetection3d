...
from mmdet3d.ops.detr3d_utils.box_util import (extract_pc_in_box3d, flip_axis_to_camera_np,
                            get_3d_box, get_3d_box_batch)
from mmdet3d.ops.detr3d_utils.eval_det import eval_det_multiprocessing, get_iou_obb
from mmdet3d.ops.detr3d_utils.nms import nms_2d_faster, nms_3d_faster, nms_3d_faster_samecls
...
def parse_predictions_mmdet(
    ...
    for i in range(bsize):
        ...
        pick = nms_3d_faster_samecls(
            boxes_3d_with_prob[nonempty_box_mask[i, :] == 1, :],
            0.25, # TODO: HARDCODING NOW, ADD TO CFG LATER            
            False, # TODO: HARDCODING NOW, ADD TO CFG LATER
            # config_dict["nms_iou"],
            # config_dict["use_old_type_nms"],
        )
        assert len(pick) > 0
